...
# 스크립트 위치(test 폴더)의 상위 경로인 프로젝트 루트를 자동으로 기준점으로 설정
$RootDir = Resolve-Path "$PSScriptRoot/.."
$MarsJar = "$RootDir/mars.jar"
$ProgramsDir = "$RootDir/programs"
$DumpsDir = "$RootDir/dumps"
$BinDir = "$RootDir/bin"
$SourcesDir = "$RootDir/chap11"
...
$Sources = Get-ChildItem -Path $SourcesDir -Recurse -Include "*.java" | ForEach-Object { $_.FullName }
if ($Sources.Count -eq 0) {
    Write-Error "No Java sources found under $SourcesDir! Ensure you run this inside the compiler root."
    Exit 1
}
...
foreach ($File in $Files) {
    ...
    java -cp $TempClassDir $File | Out-File -FilePath $ExpectedPath -Encoding ascii
    ...
    java -cp $BinDir Main $JavaFile | Out-File -FilePath $AsmPath -Encoding ascii
    if ($LASTEXITCODE -ne 0) {
    ...
    java -jar $MarsJar nc $AsmPath | Out-File -FilePath $ActualPath -Encoding ascii
    if ($LASTEXITCODE -ne 0) {
...
$MarkdownTable = "| Program Name | Java Expected Output (JVM) | MIPS Actual Output (MARS) | Correctness (Status) |`n"
$MarkdownTable += "| :--- | :--- | :--- | :---: |`n"
